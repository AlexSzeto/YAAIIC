...
- **`preGenerationTasks`** (array, optional)
  - LLM tasks or template tasks to run before ComfyUI execution.
  - See [LLM Task Object](#llm-task-object).

- **`postGenerationTasks`** (array, optional)
  - LLM tasks or template tasks to run after ComfyUI execution.
...
- **`extractOutputTexts`** (array of strings, optional)
  - Extracts text content from files in the storage folder and assigns them to generation data properties.
  - Each string in the array corresponds to both:
    - A text filename in the storage folder (e.g., `"summary"` → `"summary.txt"`)
    - A property name in the generation data (e.g., `generationData.summary`)
  - Processing occurs after workflow execution but before post-generation tasks.
  - Extracted values are available for use in post-generation prompts and templates.
  - Example:
    ```json
    "extractOutputTexts": ["summary", "tags"]
    ```
    This reads `summary.txt` and `tags.txt` from the storage folder and assigns their content to `generationData.summary` and `generationData.tags` respectively.
...
Used in `preGenerationTasks` and `postGenerationTasks` to perform arithmetic on a generation data field:
...
```json
{
  ...
  "to": "adjustedFrames",
  "math": [
    { "offset": 0, "scale": 1.5, "bias": 0, "round": "floor" }
  ]
}
```
...
- **`from`** (string, required): Source field in `generationData`.
- **`to`** (string, required): Target field in `generationData` to write the result.
- **`math`** (array, required): Ordered steps. Each step receives the output of the previous.
  - **`offset`** (number, default `0`): Added to the value before scaling.
  - **`scale`** (number, default `1`): Multiplier applied after offset.
  - **`bias`** (number, default `0`): Added after scaling.
  - **`round`** (`"none"` | `"floor"` | `"ceil"`, default `"none"`): Optional rounding.
- **`condition`** (object, optional): Task is skipped when condition is false. See [Condition Object](#condition-object).

Formula per step: `result = (value + offset) * scale + bias`, then rounded.
