...
const ActionRow = styled('div')`
  ...
  flex-wrap: wrap;
`
ActionRow.className = 'part-item-action-row'
...
export function PartItem({
  ...
  const [isRevealing, setIsRevealing] = useState(
    () => getPartsCoverage()[partKey] ?? false,
  )
  ...
  const [localPreviewBaseline, setLocalPreviewBaseline] = useState(
  ...
  useEffect(() => {
    if (!previewBaselineFocusedRef.current)
      setLocalPreviewBaseline(config.previewBaseline || '')
  }, [config.previewBaseline])
  ...
  const requestPortraitCache = useCallback(
    async (updatedPart) => {
      const prompt = assemblePartPreviewPrompt(updatedPart)
      console.log(
        '[auto-preview] requestPortraitCache called, prompt:',
        prompt || '(empty)',
      )
      if (!prompt) return
      ...
      try {
        ...
        console.log(
          '[auto-preview] cache lookup result:',
          result,
          'partUid:',
          partUid,
        )
        if (result.found) {
          onChange({
            ...updatedPart,
            data: { ...updatedPart.data, previewImageUrl: result.portraitUrl },
          })
          return
        }
        ...
        const stale = partUid
          ? queueItemsRef.current.filter(
              (i) =>
                i.endpointKey === 'anytale-part-preview' &&
                i.taskData?.partUid === partUid,
            )
          : []
        console.log(
          '[auto-preview] stale queue items:',
          stale.length,
          stale.map((i) => i.id),
        )
        await Promise.all(
          stale.map((i) =>
            fetch(`/queue/item/${encodeURIComponent(i.id)}`, {
              method: 'DELETE',
            }).catch(() => {}),
          ),
        )
        console.log(
          '[auto-preview] enqueueing fresh preview, prompt:',
          prompt,
          'partUid:',
          partUid,
        )
        const enqRes = await fetch(
          '/anytale/generate-part-preview?queueOnly=false',
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              prompt,
              partContext: 'parts-list',
              partUid,
  ...
  const handleTypeChange = useCallback(
    (newTypes) => {
      const oldTypeSet = new Set(
        (Array.isArray(config.type) ? config.type : []).map((t) =>
          t.toLowerCase(),
        ),
      )
  ...
  const handleAttrsChange = useCallback(
    (newAttrs) => {
      ...
      const cleanedValues = Object.fromEntries(
        Object.entries(data.attributeValues || {}).filter(([k]) =>
          validNames.has(k),
        ),
      )
      onChange({
        ...part,
        config: { ...config, attributes: newAttrs },
        data: { ...data, attributeValues: cleanedValues },
      })
    },
  ...
  const handleRandom = useCallback(() => {
    ...
    for (let i = 0; i < attrs.length; i++)
      newValues[attrs[i].name ?? String(i)] = ''
    ...
    for (const idx of shuffled.slice(0, count)) {
      ...
      if (opts.length > 0)
        newValues[attr.name ?? String(idx)] =
          opts[Math.floor(Math.random() * opts.length)].value
    }
    const updatedPart = {
      ...part,
      data: { ...data, attributeValues: newValues, previewImageUrl: '' },
    }
    onChange(updatedPart)
    requestPortraitCache(updatedPart)
  }, [part, data, config.attributes, onChange, requestPortraitCache])
  ...
  const handleTagImport = useCallback(
  ...
  const generateCategorizedOptions = async (
    attrIndex,
    promptTitle,
    hint,
    categorizedOptions,
    ...
    const keyword = await showTextPrompt(
      promptTitle,
      config.referenceTag.toLowerCase(),
      hint,
    )
    ...
    next[attrIndex] = {
      ...next[attrIndex],
      name: autoName,
      options: validTags.join(', '),
    }
    handleAttrsChange(next)
  }

  const handleRainbowAction = useCallback(
    async (attr, attrIndex) => {
      generateCategorizedOptions(
        attrIndex,
        'Color Keyword',
        'e.g. eyeshadow',
        RAINBOW_COLORS,
  ...
  const BANNED_VARIATION_KEYWORDS = [
    'holding',
    'holding unworn',
    'mismatched',
    'no',
  ...
  const handleVariationsAction = useCallback(
    async (attr, attrIndex) => {
      const keyword = await showTextPrompt(
        'Leftover Variation Keyword',
        config.referenceTag.toLowerCase(),
        'e.g. camisole',
      )
      if (!keyword || !keyword.trim()) return
      ...
      const validTags = getTags()
        .filter((tag) => {
          ...
          return (
            (normalised.endsWith(`-${kw}`) || normalised.endsWith(` ${kw}`)) &&
            !coveredTags.has(normalised)
          )
        })
        .map((tag) => tag.replace(/_/g, ' '))
  ...
  const getAttrSelectOptions = useCallback(
    (attr) => getAttrOptions(attr.options),
    [],
  )
  const getAttrSelectValue = useCallback(
    (attr, i) => data.attributeValues?.[attr.name] || '',
    [data.attributeValues],
  )
  const handleAttrSelectChange = useCallback(
    (attr, i, value) => handleAttrValueChange(i, value),
    [handleAttrValueChange],
  )
  const getAttrSelectTooltip = useCallback(
    (attr) => getTagDefinition(data.attributeValues?.[attr.name] || '') || null,
    [data.attributeValues],
  )
  ...
  const handleDeleteFromLibrary = useCallback(async () => {
    ...
    const result = await showDialog(
      `Delete '${config.referenceTag}' from the library? This cannot be undone.`,
      'Delete from Library',
      ['Delete', 'Cancel'],
    )
  ...
  const handleRevertToLibrary = useCallback(async () => {
    ...
    const result = await showDialog(
      'Revert this part to the saved library version? Unsaved changes will be lost.',
      'Revert Part',
      ['Revert', 'Cancel'],
    )
  ...
  const isUnchangedFromLibrary =
    !!libraryPart &&
    JSON.stringify({
      referenceTag: config.referenceTag,
      name: config.name,
      type: config.type,
      ...
      JSON.stringify({
        referenceTag: libraryPart.referenceTag,
        name: libraryPart.name,
        type: libraryPart.type,
        baseline: libraryPart.baseline,
        previewBaseline: libraryPart.previewBaseline,
        attributes: libraryPart.attributes,
      })
  const { saveLabel, saveEnabled, deleteEnabled, revertEnabled } =
    formButtonStates(!!libraryPart, !isUnchangedFromLibrary)
  ...
  return html`
    <${VerticalLayout} gap="medium">
      ...
      <${TagInput}
        ...
        onFocus=${() => {
          previewBaselineFocusedRef.current = true
        }}
        onBlur=${() => {
          previewBaselineFocusedRef.current = false
          const updatedPart = {
            ...part,
            config: { ...config, previewBaseline: localPreviewBaseline },
          }
          onChange(updatedPart)
          requestPortraitCache(updatedPart)
        }}
      ...
      <${TagInput}
        ...
        onFocus=${() => {
          baselineFocusedRef.current = true
        }}
        onBlur=${() => {
          baselineFocusedRef.current = false
          const updatedPart = {
            ...part,
            config: { ...config, baseline: localBaseline },
          }
          onChange(updatedPart)
          requestPortraitCache(updatedPart)
        }}
      ...
      <!-- Library Actions -->
      <${ActionRow} style=${{ marginTop: currentTheme.value.spacing.small.gap }}>
        <${Button}
          variant="small-text"
          color="primary"
          icon="save"
