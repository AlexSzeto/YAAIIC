...
1. Prepare for the next set of tasks by creating a new `.md` file inside `docs/feature-history/` with an appropriately named filename based on the title text inside `task.md`, similar to other existing completed tasks.
2. The file should start with a number indicating the order the task was completed relative to the existing files.
3. Copy the title, goals, related notes (if any), and tasks inside `task.md` that are marked as completed verbatim into the newly created file inside `docs/feature-history/`.
