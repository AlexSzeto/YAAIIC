...
- Add AnyTale dialog settings under `server/config.default.json` / runtime config, for example: dialog model, mode (`chat` or `completion`), completion format (`chatml` when needed), stream default, base system prompt path or inline prompt, and Ollama `options` (temperature/top_p/etc.).
- Fetch the AnyTale dialog config through `GET /anytale/config`; do not hard-code model names in the play client.
- Add a small AnyTale dialog client module that builds messages from the configured base prompt, character personality, and page `dialogPrompt`, then calls `/api/chat`. Prefer non-streaming for the first usable implementation unless the UI is explicitly wired for streaming partial text.
...
- [ ] **AnyTale dialog config:** Extend the AnyTale config returned by `GET /anytale/config` with dialog settings: model, mode (`chat` or `completion`), optional `format: "chatml"`, stream default, base system prompt or prompt file path, and Ollama options. Keep defaults in `server/config.default.json` and avoid hard-coded play-client model names. **Manual test:** change the configured model/options and verify the play client sends the new values without code edits.

- [ ] **AnyTale chat client adapter:** Add a play-mode client module that calls the imported `POST /api/chat` endpoint. It should build messages from base AnyTale instructions + character personality + page `dialogPrompt`, support the configured chat/completion mode, normalize the response shape, and expose a single `generateDialog({ character, page, signal })` style API for the UI. **Manual test:** call the adapter from a page with a dialog prompt and verify the speech bubble is filled from `/api/chat`.

- [ ] **Dialog generation flow:** Wire page-entry dialog generation to the adapter. When dialog is off, `dialogPrompt` is blank, or character personality is blank, skip `/api/chat` entirely and clear/retain UI according to the page-entry rule. Cache successful dialog text in the generated asset cache by stable signature. **Manual test:** toggle dialog off, advance pages, no network to `/api/chat`; blank page dialog prompts also produce no chat request; back navigation reuses cached dialog.
