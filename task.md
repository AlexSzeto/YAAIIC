...
### `extraInputs` object
All dynamic workflow-specific fields live here, including:
- Custom workflow parameters (e.g. `imageFormat`, `usePostPrompts`, `removeBackground`)
- Input media references (e.g. `image_0_uid`, `audio_0_uid`)

The contents are validated against the workflow's `options.extraInputs` array and `options.inputImages`/`options.inputAudios` counts at save time.

### Migration backup
Since `media-data.json` is not version-controlled, the migration script creates a timestamped copy in `scripts/migrate/backups/` before making any changes. This folder should be added to `.gitignore`.

### Fields explicitly removed by migration
These are generation working data that should never persist:
- `saveImagePath`, `saveAudioPath`, `saveImageFilename`
- `storagePath`, `imagePath`, `maskPath`
- `ollamaAPIPath`