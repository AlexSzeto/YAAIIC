...
[] Implement the UID query param for the index page, `uid=XXX`. When a UID is provided, on page load, populate the gallery data with a single entry from the `uid` by loading its associated data.
...
[] Implement a "Done" button in the inpaint page and place it to the right of the "Inpaint" button. The button links back to the index page and sends the most recent `uid` as a query parameter.
