...
[] (Client) Add support for `type: "video"` on the client main UI. Treat the workflow identically to image workflows except to enable two more number form values between the existing `Name` and `Seed` fields: `Length` (in seconds, sent to the server as `frames`, which is `length*framerate`), and `Frame Rate` (sent to the server as `framerate`)
...
4. Add two new number input fields: `Length` (in seconds) and `Frame Rate` (in fps)
5. Position these fields between the `Name` and `Seed` fields in the form
6. Implement client-side calculation logic to convert `Length * Frame Rate = frames` before sending to server
