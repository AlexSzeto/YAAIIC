...
- [ ] Clip regions are still being removed from the timeline whenever a new active region is selected.
- [ ] Playbacks are not working as intended - when a region is selected, playback is not stopping at the end of the selected area. The loop button doesn't work at all.
- [ ] Change the waveform progress color to be the primary contrast color (white in dark mode, black in light mode)
