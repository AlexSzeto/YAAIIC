...
- [ ] Add `arrow-right-stroke` to the icon map in `public/js/custom-ui/layout/icon.mjs`, mapping it to the Material Symbol `arrow_right_alt`. Place the entry in the Verified section at the bottom of `ICON_MAP`.
...
- [ ] Rename `public/js/custom-ui/nav/hamburger-menu.mjs` to `navigation-menu.mjs`. Rename the exported component `NavPanel` to `NavigationMenu` within that file. Update the import in `public/js/app-ui/hamburger-menu.mjs` to use the new path and component name.
...
- [ ] Add `HamburgerMenu` to the Workflow Editor page header. In `public/js/app-ui/workflow-editor.mjs`, import `HamburgerMenu` from `'./hamburger-menu.mjs'` and render it inside the existing header `<div>` (the flex row alongside the `H1` title), placed before the title so it appears on the left edge.
...
- [ ] Fix single-object condition parsing in `ConditionBuilder`. In `public/js/app-ui/condition-builder.mjs`, add a normalization step at the top of `ConditionBuilder` that detects a bare condition object (one that has a `where` key but lacks an `and` or `or` wrapper) and wraps it as `{ and: [value] }` before deriving `mode` and `conditions`.
...
- [ ] Add "Additional processing" task type to `TaskForm`. In `public/js/app-ui/task-form.mjs`:
