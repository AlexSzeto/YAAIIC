...
- [x] Create `pan.mjs` with `PanEffect` class using the implementation of gain range as a reference for how they are handled during loops and events — wrap a `StereoPannerNode`. Expose `get input`, `get output`, `setActive(panConfig, duration)`, `connect(dest)`, `disconnect()`, `dispose()`. The `panConfig` is an object `{ mode, value?, min?, max? }` or `null` (center/off).
...
- [x] Refactor channel gain to a single master volume — `AmbientChannel`: replace `#gainRange` with `#gain` (number, 0–1) that controls `#output.gain`. Constructor backward compat: old `gain: { min, max }` → midpoint; old `distance` string → midpoint of `DISTANCE_GAIN_MAP`. `setEnabled` ramps to `#gain` (not 1.0) when re-enabling. Remove `setGainRange()` and `setPan()`. Add `setGain(value)` and `getTrack(i)`. Remove `PanEffect` from the effect chain entirely. Remove forwarding `gainRange` to tracks (tracks now own their gain). Update `createDefaultChannel()`: `gain: 0.5`. Update `handleChannelLiveUpdate` to call `ch.setGain(next.gain)`.

- [x] Move gain range and pan to individual tracks — `LoopingTrack` and `EventTrack`: each constructor accepts `gain: { min, max }` and `pan: panConfig` from track data, initialising its own `#gainRange` (`Range.fromData`) and `#panConfig`. Each track creates a `#panner` (`StereoPannerNode`) in `playInto`, wiring `#gain → #panner → destination`; `setPanConfig(panConfig)` updates `#panConfig` and `#panner` for fixed/off. Backward compat: missing `gain` → `{ 0.5, 0.5 }`, missing `pan` → `null`. `handleChannelLiveUpdate` in `brew-editor.mjs` iterates tracks and calls `track.setGainRange` / `track.setPanConfig` for per-track live updates. Also fix the `#playing` flag in `LoopingTrack` (never set to `true`).

- [x] Add gain range and pan UI to `TrackForm` — `track-form.mjs`: add gain `RangeSlider` (0–1, snap 0.01) and pan `DiscreteSlider` with conditional `Input`/`RangeSlider` identical to what was previously on `channel-form.mjs`. Loop tracks show pan modes Off + Fixed only. Update `createTrack()` defaults: `gain: { min: 0.5, max: 0.5 }`, `pan: null`. `channel-form.mjs`: replace the gain `RangeSlider` with a number `Input` (0–1, step 0.01); remove pan controls.

- [x] Implement random pan per event and sweeping pan modes — in `EventTrack.#playEventLoops`, for `random` mode create a per-event `StereoPannerNode`, pick a random value from `[min, max]` and set it at `when`. For `left-to-right`: ramp –1 → +1 over the event's duration. For `right-to-left`: ramp +1 → –1. Per-event panners connect between `eventGain` and `#gain`, and are disconnected after the event completes.
...
- [x] Create a new UI in custom-ui/io, `slider.mjs`, that replicates the input for range-slider except that its output would be a single value. the manual input value would be placed on the bottom left edge, meaning it should be in the same place as `range-slider`'s min value. Otherwise, keep the overall layout exactly the same as `range-slider`, including the margin for the slider line normally reserved for the right input, so vertically stacked `slider`s and `range-slider`s would line up correctly.

- [x] Convert all discrete sliders for panning into select inputs.

- [x] Implement random panning for looping tracks - similar to variable gain, each loop starts and ends at a random pan value between the user set range and the audio linearly pans from the start value to the end value, and each new loop reuses the previous end value as start value and picks a new end value.

- [x] For fixed gain and panning inputs, convert the number input into a custom-ui slider instead. Remove the "off" option for panning and default it to "fixed" with a value of "0".
...
- [x] Fix a bug in the audio editor modal where clip regions are not being saved when a clip is trimmed or clipped in the same edit session, even though the regions appear correctly in the timeline before they are saved. This may be related to most of the generated data being overwritten by the album generation workflow - the tags/prompt/description/summary are not ported over from the previous audio either. This might mean the edit needs an upload endpoint that accepts more parameters (data properties from the old audio entry) that can be entered directly into the new uploaded entry.

(NOTE: needs manual testing to verify)

- [x] Add uids (using Date.now() like other uids) to the Ambient Brews so when its name changes and saves, it would save over
