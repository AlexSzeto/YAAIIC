...
- [x] **Voice replays when next page TTS completes:**

- [x] **BGM doesn't restart after toggle off → on:** `globalBgmPlayer.stop()` clears the playlist. When `session.musicOn` flips back to true, the sync effect calls `play()` on an empty playlist (no-op). Fix: reload the playlist from the session genre inside the sync effect before calling `play()` when the playlist is empty.

- [x] **Persist dialog/music preferences across resets:** Extract `muted` and `musicOn` from the play session into a separate `anytale-play-prefs` localStorage key. Session resets (`handleReset`) clear `anytale-play-session` but leave `anytale-play-prefs` intact. On session load, prefs values override the session defaults for these keys. Toggling mute or music saves to both session and prefs. The page-ready effect fires whenever any page's voice URL is set (pageVoiceUrls dep). When the effect re-fires for a page already fully displayed, it calls `globalAudioPlayer.play()` again, restarting audio. Fix: only transition image and start voice when the displayed URL is actually changing (`imgUrl !== displayedImageUrl`). Also move image preload before the voiceSettled gate so browser cache warms while TTS is still generating. Restore unmute voice play in mute toggle handler (it was previously handled by the page-ready effect but the new guard blocks it).