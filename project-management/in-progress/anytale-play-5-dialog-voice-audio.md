...
- [x] **Voice replays when next page TTS completes:** The page-ready effect fires whenever any page's voice URL is set (pageVoiceUrls dep). When the effect re-fires for a page already fully displayed, it calls `globalAudioPlayer.play()` again, restarting audio. Fix: only transition image and start voice when the displayed URL is actually changing (`imgUrl !== displayedImageUrl`). Also move image preload before the voiceSettled gate so browser cache warms while TTS is still generating. Restore unmute voice play in mute toggle handler (it was previously handled by the page-ready effect but the new guard blocks it).