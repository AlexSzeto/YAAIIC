...
- [x] **Chapter and page labels:** Display `Chapter X  Page Y` in the bottom-left area below the progress bar. Chapter number derived from timeline position. **Manual test:** verify labels update on navigation and match expected values.

- [x] **Show/hide UI wired:** Connect the show/hide toggle button to hide/show all top controls, speech bubble area, and bottom bar. When hidden, only the show button remains visible. **Manual test:** hide UI, verify only show button visible; show UI, verify all controls return.
