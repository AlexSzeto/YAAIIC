...
- [x] Add test coverage for the new `render-outfit` endpoint and the `render-portrait` rename in `router.test.mjs`.

### Phase 4 — Frontend

- [x] Update `anytale-state.mjs`: add `preferredLocations: []` and `renderUrl: ''` to `createBlankOutfit()` and the `loadOutfit()` deserializer.

- [x] Update `outfit-section.mjs`:
...
- [x] Add `renderOutfit(uid)` to `outfit-api.mjs`.

- [x] Review and update affected living docs: `docs/features/anytale.md`, `docs/server.md`

#### Fixes and Changes

- [x] Restore missing `server/resource/media-data-schema.json` accidentally deleted in the documentation audit commit (`5754d5d`); this caused all generation tasks that write to media-data to crash with ENOENT in `sanitizer.mjs`.

- [x] Apply slot layer rules to outfit render prompt assembly: client-side in `outfit-section.mjs`, before calling `renderOutfit()`, build `activeParts` from outfit parts resolved via `libraryParts`, call `resolveSlotStatuses(activeParts, [], -1)` (no plot pages — initial state defaults all slots to covering/revealing per `isRevealing` config), then `applyRules(slotStatuses, parsedRules)` to get visibility. Filter to `visiblePartUids` (parts where at least one slot type is visible). Pass `visiblePartUids` in the request body. Extend `parsedRules` state to be populated from the existing config fetch (`slotRules`). Update `renderOutfit(uid, visiblePartUids)` in `outfit-api.mjs` to pass the body. Update the server handler in `router.mjs` to accept `visiblePartUids` from the request body and filter `outfit.parts` before prompt assembly.
