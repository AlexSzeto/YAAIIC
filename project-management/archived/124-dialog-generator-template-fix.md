...
- [x] Manually verify: open the AnyTale editor, configure `dialogPreview` with `name`/`profile`, load a plot with a `dialogPrompt`, click "Preview Dialog" — the generated dialog should reflect the character profile and visible outfit parts; confirm no literal `{{outfit}}` or `{{profile}}` appears in the Ollama request log
