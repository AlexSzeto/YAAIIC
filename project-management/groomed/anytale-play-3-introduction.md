...
- [ ] **Cold start bootstrap:** On first visit (no session in localStorage), randomly select: character (from library), outfit (from character's preferred outfits), background part + random attribute values (from background-typed parts, respecting category/custom attribute option lists), music genre + random track, prelude plot (`section === 'prelude'`). Store all selections in session. **Manual test:** clear localStorage, open play page, verify all selections are populated in session and different across multiple clears.
...
- [ ] **Introduction image generation:** On cold start, assemble prompt from current character parts + outfit parts + background part + introduction plot page tags using `assemblePrompt`. Trigger image generation via the config-selected AnyTale workflow. Show loading spinner during generation. Display result in portrait area. **Manual test:** verify generated image appears after loading; image reflects the selected character/outfit/background.
...
- [ ] **Introduction mood page:** Three options (no fourth): 'Maybe try on a different outfit?' → outfit change (3+1), 'Let's go somewhere else.' → background change, 'Let's listen to something different.' → music change. Completing any change returns to intro main page. **Manual test:** navigate to mood, pick each option, verify return to main page after completion.
...
- [ ] **Background change flow:** Show three random background-typed parts. On pick, walk each attribute: three concrete options from the hydrated option list + fourth 'None of these' (leaves attribute null). After all attributes, update session background, regenerate intro image (block + fade). Return to intro main page. **Manual test:** skip an attribute with 'None of these', confirm the generated prompt omits that tag value.

- [ ] **Music genre selection:** Show three genres from the music database. On pick, select a random track from that genre's playlist, begin playback with graceful crossfade. Return to mood page. Does NOT trigger image regeneration. **Manual test:** pick a genre, verify music starts playing; pick another genre, verify crossfade transition.
...
```js
{
  ...
  outfitUid: 'outfit-...',
  background: { partUid, attributeMap: { attrName: 'value' | null } },
  music: { genre: 'fantasy', trackId: '...' },
  preludePlotUid: 'plot-...',
  phase: 'intro-main' | 'intro-mood' | 'character-pick' | 'outfit-pick' | 'background-pick' | 'music-pick',
...
| Background parts | `GET /anytale/parts` | `type` array includes `'background'` (case-insensitive) |
| Plots | `GET /anytale/plot` | Full list; filter `section === 'prelude'` (case-insensitive) |
| Config | `GET /anytale/config` | Read `introductionPlotName`, `generationWorkflow` |

If any required category is empty (no characters, no background parts, no prelude plot, no introduction plot), show a blocking error message directing the user to create the missing data in the AnyTale editor.
...
// enabledParts = character parts + outfit parts + background part
...
const enabledParts = [
  ...
  backgroundPart,     // from session background with attributeMap applied
];
...
### Background attribute wizard

When the user picks a background part, walk each attribute sequentially. For each attribute:
...
- After all attributes are resolved, update the session `background.attributeMap` and regenerate.
...
Use the two-channel audio runtime if built in a prior rollout, or a simple placeholder `<audio>` element that will be replaced in Rollout 5. Crossfade between tracks by fading out the current channel over ~1s while fading in the new channel. Music changes do **not** trigger image regeneration.
...
intro-main → intro-mood → background-pick → intro-main
