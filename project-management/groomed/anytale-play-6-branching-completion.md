...
- [ ] **End-of-chapter decision page:** When the user reaches the last page of a chapter, show a decision page. Reuse image from the last page. Display a decision hint in the caption bubble. Offer three plots whose `section` matches one of the current plot's `progressionSections` entries. Fourth option: "Let's say goodbye for now" → random epilogue plot (`section === epilogue`, case-insensitive). If no progression sections are defined or no matching plots exist, show only the epilogue option. **Manual test:** reach end of chapter with fixture plots, verify correct section filtering; test empty `progressionSections` shows only epilogue.
...
- [ ] **Timeline persistence:** Append chosen plots to the linear timeline as ordered entries: `{ plotUid, startedAt, pageCount, progressionDisabledPartsApplied }`. Persist in the session `localStorage` object. On reload, restore the timeline and allow navigation across all chapters in the session. **Manual test:** branch through multiple chapters, reload, verify timeline is preserved and cross-chapter navigation works.

- [ ] **progressionDisabledParts enforcement:** When a plot is selected and added to the timeline, apply its `progressionDisabledParts` to all subsequent prompt assembly by suppressing matching part names/types from `enabledParts`. This accumulates across chapters until reset. **Manual test:** create a plot with `progressionDisabledParts`, verify that the disabled part is omitted from image prompts in subsequent chapters.
...
- [ ] **Reset UX:** Wire the reset button: show confirmation modal, on confirm: cancel all in-progress generations (dialog, TTS, images, pre-renders) via the cancellation infrastructure, stop all playing audio (voice + music via two-channel runtime), perform full cold start reroll (same rules as first visit — random character, outfit, background, music, prelude). **Manual test:** mid-chapter, tap reset, confirm dialog, verify audio stops immediately, new session loads, intro appears with fresh random selections.
