...
- [ ] Add `startConfigWatcher(onChange)` to `server/core/config.mjs`:
...
- [ ] Add `server/core/config.test.mjs` with tests for `startConfigWatcher`:
...
- [ ] Export `reconnectComfyUIWebSocket(newApiPath)` from `server/comfyui-websocket.mjs`:
  - Set `comfyUIAPIPath = newApiPath`
  - Call `connectToComfyUI(true)` (force teardown and reconnect)
- [ ] In `server.mjs`, call `startConfigWatcher` inside `startServer()` after the server begins listening; wire the `onChange` callback to:
...
- [ ] Review and update affected living docs: `docs/server.md`
